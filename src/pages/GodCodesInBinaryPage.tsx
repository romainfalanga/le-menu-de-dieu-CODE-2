...
const GodCodingSimulation: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (isTyping && currentCode.length < currentSequence.length) {
      ...
      }, 1.3); // Multiplié par 5 pour ralentir l'écriture de l'ensemble de la série
    ...
    if (isTyping && currentCode.length === currentSequence.length) {
      ...
      const pauseTimer = setTimeout(() => {
        setIsTyping(false);
        setIsDeleting(true);
      }, 10000); // Attendre 10 secondes avant de commencer la suppression (multiplié par 5)
    ...
    if (isDeleting && currentCode.length > 0) {
      ...
      }, 4); // Suppression ralentie (multiplié par 5)
